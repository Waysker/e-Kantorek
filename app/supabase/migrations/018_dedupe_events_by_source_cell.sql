...
with ranked as (
  select
    ...
    first_value(e.event_id) over (
      ...
      order by
        -- Prefer canonical titles without embedded dates.
...
-- Move attendance to canonical event ids.
...
-- Move queue rows where possible.
...
-- Remove old event rows (remaining dependent queue rows are cascaded).
delete from public.events e
using tmp_event_source_dedupe_map map
where e.event_id = map.old_event_id;

-- Prevent future duplicates on the same sheet cell.
