create table if not exists public.attendance_sheet_cache (
  attendance_key text primary key,
  payload jsonb not null,
  generated_at timestamptz not null,
  updated_at timestamptz not null default now()
);

create or replace function public.touch_attendance_sheet_cache_updated_at()
returns trigger
language plpgsql
as $$
begin
  new.updated_at = now();
  return new;
end;
$$;

drop trigger if exists trg_attendance_sheet_cache_updated_at on public.attendance_sheet_cache;
create trigger trg_attendance_sheet_cache_updated_at
before update on public.attendance_sheet_cache
for each row
execute function public.touch_attendance_sheet_cache_updated_at();

alter table public.attendance_sheet_cache enable row level security;

drop policy if exists "Allow authenticated read attendance sheet cache" on public.attendance_sheet_cache;
create policy "Allow authenticated read attendance sheet cache"
on public.attendance_sheet_cache
for select
to authenticated
using (true);

drop policy if exists "Allow privileged insert attendance sheet cache" on public.attendance_sheet_cache;
create policy "Allow privileged insert attendance sheet cache"
on public.attendance_sheet_cache
for insert
to authenticated
with check (
  exists (
    select 1
    from public.profiles p
    where p.id = auth.uid()
      and p.role in ('leader', 'admin')
  )
);

drop policy if exists "Allow privileged update attendance sheet cache" on public.attendance_sheet_cache;
create policy "Allow privileged update attendance sheet cache"
on public.attendance_sheet_cache
for update
to authenticated
using (
  exists (
    select 1
    from public.profiles p
    where p.id = auth.uid()
      and p.role in ('leader', 'admin')
  )
)
with check (
  exists (
    select 1
    from public.profiles p
    where p.id = auth.uid()
      and p.role in ('leader', 'admin')
  )
);

grant select, insert, update on table public.attendance_sheet_cache to authenticated;

drop policy if exists "Allow privileged insert forum instrument overrides" on public.forum_instrument_overrides;
create policy "Allow privileged insert forum instrument overrides"
on public.forum_instrument_overrides
for insert
to authenticated
with check (
  exists (
    select 1
    from public.profiles p
    where p.id = auth.uid()
      and p.role in ('leader', 'admin')
  )
);

drop policy if exists "Allow privileged update forum instrument overrides" on public.forum_instrument_overrides;
create policy "Allow privileged update forum instrument overrides"
on public.forum_instrument_overrides
for update
to authenticated
using (
  exists (
    select 1
    from public.profiles p
    where p.id = auth.uid()
      and p.role in ('leader', 'admin')
  )
)
with check (
  exists (
    select 1
    from public.profiles p
    where p.id = auth.uid()
      and p.role in ('leader', 'admin')
  )
);

grant select, insert, update on table public.forum_instrument_overrides to authenticated;
